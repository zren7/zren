...
import { getElementVisibleRect, type VisibleDomRect } from './dom'
...
export function useLayoutHeaderStyle() {
  ...
  return {
    getLayoutHeaderHeight: () => {
      return Number.parseInt(String(headerHeight.value), 10)
    },
...
export function useLayoutFooterStyle() {
  ...
  return {
    getLayoutFooterHeight: () => {
      return Number.parseInt(String(footerHeight.value), 10)
    },
